...
namespace MoviesAPI.Tests.UnitTests
{
    ...
    public class PeopleControllerTests : BaseTests
    {
        ...
        public async Task GetPeoplePaginated()
        {
            ...
            var responsePage1 = await controller.GetPerson(new PaginationDTO() { Page = 1, RecordsPerPage = 2 });
            var peoplePage1 = responsePage1.Value;
            Assert.AreEqual(2, peoplePage1.Count());

            controller.ControllerContext.HttpContext = new DefaultHttpContext();

            var responsePage2 = await controller.GetPerson(new PaginationDTO() { Page = 2, RecordsPerPage = 2 });
            var peoplePage2 = responsePage2.Value;
            Assert.AreEqual(1, peoplePage2.Count());

            controller.ControllerContext.HttpContext = new DefaultHttpContext();

            var responsePage3 = await controller.GetPerson(new PaginationDTO() { Page = 3, RecordsPerPage = 2 });
        ...
        public async Task CreatePersonWithoutImage()
        {
            ...
            var response = await controller.PostPerson(newPerson);
        ...
        public async Task PatchReturns404IfPersonNotFound()
        {
            ...
            var response = await controller.PatchPerson(1, patchDoc);
